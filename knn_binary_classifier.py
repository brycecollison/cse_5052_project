...
# load data set
# change to relative path to data file
...
def euclidean_distances(test_x, train):
    distances = [np.sqrt(np.sum((test_x - train[:, i]) ** 2))
...
class KNearestNeighbors:
    ...
    def _predict_one(self, test_x):
        ...
        # return most frequent label from k-nearest neighbors to test data point in training data set
        return np.bincount(k_nearest_labels).argmax()

    # predict labels of a given set of data points
    def predict(self, test_x):
        predictions = [self._predict_one(test_x[:, i])
...
for i, idx in enumerate(mislabeled_indices):
    print(f"Index: {idx}, True Label: {true_labels[i]}, Predicted Label: {
          predicted_labels_mislabeled[i]}")
...
for i, idx in enumerate(mislabeled_indices[:5]):
    ...
    plt.title(f"True: {true_labels[i]}, Predicted: {
              predicted_labels_mislabeled[i]}")
    plt.axis('off')

    # save the mislabeled image
    image_path = os.path.join(output_dir, f"mislabeled_{idx}_true_{
                              true_labels[i]}_pred_{predicted_labels_mislabeled[i]}.png")
...
correct_4_indices = np.where(np.logical_and(
    predicted_labels == y_test, predicted_labels == 0))[0]
correct_4 = x_test[:, correct_4_indices]
correct_9_indices = np.where(np.logical_and(
    predicted_labels == y_test, predicted_labels == 1))[0]
...
example_images = np.concatenate(
    (correct_4[:, :5], correct_9[:, :5], mislabeled_data[:, :5]), axis=1)
example_indices = np.concatenate(
    (correct_4_indices[:5], correct_9_indices[:5], mislabeled_indices[:5]))


# create figure with 5 correctly labeled images of each class and 5 incorrectly labeled images
fig = plt.figure(figsize=(10, 10), layout="constrained")
fig.suptitle(f"Classified Images (k = {optimal_k}; accuracy = {
             test_accuracy})", fontsize=24)
...
for row, subfig in enumerate(subfigs):
    ...
    for col, ax in enumerate(axs):
        ...
        ax.axis("off")
        ax.set_title(f"Predicted: {predicted_labels[example_indices[subfig_index]]} True: {
                     y_test[example_indices[subfig_index]]}", fontsize=12)
